...
# mq-edit installation script

readonly MQ_REPO="harehare/mq-edit"
...
EOF
    echo -e "${BOLD}${CYAN}  TUI editor for Markdown with mq query support${NC}"
    echo ""
    echo -e "${PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
}
...
get_download_url() {
    ...
    echo "https://github.com/$MQ_REPO/releases/download/$version/mq-edit-${target}${ext}"
}
...
# Download and install mq-edit
install_mq_edit() {
    local version="$1"
    local os="$2"
    local arch="$3"
    local download_url
    local binary_name="mq-edit"
    local ext=""
    local target=""

    if [[ "$os" == "windows" ]]; then
        ext=".exe"
        binary_name="mq-edit.exe"
        target="${arch}-pc-windows-msvc"
    elif [[ "$os" == "darwin" ]]; then
        target="${arch}-apple-darwin"
    else
    ...
    log "Downloading mq-edit $version for $os/$arch..."
    ...
    if ! curl -L --progress-bar "$download_url" -o "$temp_file"; then
        error "Failed to download mq-edit binary"
    fi

    # Verify checksum
    local release_binary_name="mq-edit-${target}${ext}"
    ...
    log "mq-edit installed successfully to $MQ_BIN_DIR/$binary_name"
}

# Add mq-edit to PATH by updating shell profile
update_shell_profile() {
    ...
    if [[ -n "$shell_profile" ]]; then
        ...
        if ! grep -q "$MQ_BIN_DIR" "$shell_profile" 2>/dev/null; then
            echo "" >> "$shell_profile"
            echo "# Added by mq-edit installer" >> "$shell_profile"
...
# Verify installation
verify_installation() {
    if [[ -x "$MQ_BIN_DIR/mq-edit" ]] || [[ -x "$MQ_BIN_DIR/mq-edit.exe" ]]; then
        log "✓ mq-edit installation verified"
        log "Installation verification successful!"
        return 0
    else
        error "mq-edit installation verification failed"
    fi
}
...
show_post_install() {
    echo ""
    echo -e "${PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${BOLD}${GREEN}✨ mq-edit installed successfully! ✨${NC}"
    echo -e "${PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    ...
    echo -e "     ${CYAN}mq-edit --version${NC}"
    echo ""
    echo -e "  ${YELLOW}3.${NC} Get help:"
    echo -e "     ${CYAN}mq-edit --help${NC}"
    echo ""
    echo -e "${BOLD}${CYAN}⚡ Quick Examples:${NC}"
    echo -e "  ${GREEN}▶${NC} ${CYAN}mq-edit README.md              # Open Markdown file in editor${NC}"
    echo -e "  ${GREEN}▶${NC} ${CYAN}echo \"# Hello\" | mq-edit        # Pipe markdown content${NC}"
    echo ""
...
main() {
    ...
    # Install mq-edit
    install_mq_edit "$version" "$os" "$arch"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        --help|-h)
            echo "mq-edit installation script"
            echo ""
